...
def get_notifiers() -> Set[str]:
    ...
    return notifiers.intersection({"zeromq", "webhook", "agent", "cluster"})
...
def notify_cluster(agent: Dict[str, Any]) -> None:
    print(agent)
    try:
        k8sconfig.load_incluster_config()
    except Exception as e:
        logger.error("Error loading in-cluster configuration: %s", e)
        return
    ...
    try:
        ...
        client.CustomObjectsApi().patch_namespaced_custom_object(
            ...
            namespace="keylime",
            plural="edgenodes",
            name=agent["agent_id"]+"-edgenode",
            body=patch)
        
        
    except Exception as e:
        logger.error("Error setting node status: %s", e)
        return
