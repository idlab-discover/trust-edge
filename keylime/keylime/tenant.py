...
class Tenant:
    ...
    def init_add(self, args: Dict[str, Any], policy_file = None, data_file = None) -> None:
        ...
        ) = policies.process_policy(cast(policies.ArgsType, args), policy_file)

        # if none
        if args["file"] is None and args["keyfile"] is None and args["ca_dir"] is None and policy_file is None:
            raise UserError(
        ...
        if args["file"] is not None:
            ...
            ret = user_data_encrypt.encrypt(data_file.stream.read())
            self.K = ret["k"]
            self.U = ret["u"]
            self.V = ret["v"]
            self.payload = ret["ciphertext"]
...
def main(command=None,policy_file=None, uuid=None, payload=None) -> None:
    """[summary]
    ...
    if command is not None and command == "add":
        print("executing command with payload and policy")
        print(policy_file)
        mytenant.init_add(vars(args), policy_file, payload)
        mytenant.preloop()
        mytenant.do_quote()
        mytenant.do_cvadd()
        if args.verify:
            mytenant.do_verify()
    elif command is not None and command == "update":
        mytenant.init_add(vars(args), policy_file, payload)
        ...
        mytenant.init_add(vars(args), policy_file)
        mytenant.do_cvdelete(args.verifier_check)
        mytenant.preloop()
        mytenant.do_quote()
        mytenant.do_cvadd()
